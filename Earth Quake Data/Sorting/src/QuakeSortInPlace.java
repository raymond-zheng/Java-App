...
public class QuakeSortInPlace {
    ...
    public void onePassBubbleSort(ArrayList<QuakeEntry> quakeData, int numSorted){
    	for(int i = 0 ;  i< quakeData.size() - numSorted - 1; i ++){
    		...
    		if(cur.getMagnitude() > next.getMagnitude()){
    			quakeData.set(i, next);
    			quakeData.set(i+1,cur);
    		}
    	}
    }
    
    public void sortByMagnitudeWithBubbleSort(ArrayList<QuakeEntry> in ){
    	//should call onePassBubbleSort size - 1 times
...
//    		}
    ...
    public void sortByMagnitudeWithBubbleSortWithCheck(
    	...
    	for(int i = 0 ; i < in.size() - 1; i ++ ){
    		...
    		if(checkInSortedOrder(in)){
    			System.out.println("Used " + (i + 1)  + " passes of bubble sort");
    			break;
    		}
    	}
    ...
    public void sortByMagnitudeWithCheck(ArrayList<QuakeEntry> in){
    	for (int i=0; i< in.size(); i++) {
            ...
            in.set(minIdx,qi);
            if(checkInSortedOrder(in)){
            	System.out.println("used "+ (i+1) +" passes to finish sorting");
            	break;
            }
        }
